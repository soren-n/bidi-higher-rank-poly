...
let _num_lines layout return =
  let rec _visit layout result return =
    ...
    | Fix layout1 | Seq layout1 | Grp layout1
    | Nest layout1 | Mark (_, layout1) | Move (_, layout1) ->
...
let spec_normalize_sound =
  QCheck.Test.make ~count:1024
    ...
    (fun eDSL ->
      ...
      Spec.desugar doc @@ fun layout ->
      Spec.normalize layout @@ fun layout1 ->
...
let impl_spec_identity =
  QCheck.Test.make ~count:4096
    ~name:"impl_spec_identity"
    QCheck.(triple
      arbitrary_eDSL
      small_nat
      small_nat)
