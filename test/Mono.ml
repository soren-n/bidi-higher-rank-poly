...
and _gen_proper_simple_mono proper_simple vs =
  ...
  | SUnit ->
    ...
    ; 2, return proper_simple_mono_unit
    ]
  | SArrow (dom, codom) ->
    frequency
    [ 1, _gen_proper_simple_mono_exist proper_simple vs
    ; 2, map2 proper_simple_mono_arrow
...
let rec _gen_mono n ps vs =
  let open QCheck.Gen in
  let _gen_mono_var_fresh st =
    let exist = ref None in
    vs := exist :: !vs;
    return (mono_var exist) st
  in
  let _gen_mono_var =
    let _vs = !vs in
    let m = List.length _vs in
